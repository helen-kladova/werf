...
Секция мета-информации, это **обязательная** секция конфигурации, содержащая ключи `project: PROJECT_NAME` и `configVersion: CONFIG_VERSION`. 
...
Имя проекта влияет на имена образов в сборочном кэше, namespace в Kubernetes, имя Helm-релиза и зависящие от него имена (смотри подробнее про [развертывание в Kubernetes]({{ site.baseurl }}/ru/documentation/reference/deploy_process/deploy_into_kubernetes.html)). 
...
В одном файле конфигурации `werf.yaml` может быть несколько секций образов.
...
Образ артефакта — это вспомогательный образ, предназначенный для изолирования сборочного окружения, инструментов и данных, от конечного образа приложения (подробнее [здесь]({{ site.baseurl }}/ru/documentation/configuration/stapel_artifact.html)).
В одном файле конфигурации `werf.yaml` может быть несколько секций артефактов.

Секция артефакта определяется по наличию в секции конфигурации ключа `artifact: IMAGE_NAME`, где `IMAGE_NAME` — короткое имя Docker-образа, на которое можно ссылаться из других секций конфигурации. 
Это имя должно быть уникальным в пределах одного файла конфигурации `werf.yaml`.
...
> **Совет:** вы можете скачать или сгенерировать шаблоны до непосредственного запуска Werf. 
